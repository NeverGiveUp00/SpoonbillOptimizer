    ...
    for i = 1:N
        if rand < rand * (It / MaxIter)
            Candidate = (Ub + Lb + 2 .* BestPop - 2 .* Pops(i,:)) / 2 + ...
                        rand * abs(randn) * (BestPop - Pops(i, :)); %Equation 4
        else
            if rand > rand
                S = randperm(N, 2);
                ...
                OffsetY = -R   .* ( sin(-100 * It / MaxIter * 2 * pi)) .* BinaryArray;%Equation 3 and 5
                FullArray = (OffsetX + OffsetY);  FullArray(randi(dim)) = 0;
                Candidate = Pops(i, :)+(1 - exp(-5 * (It+20) / MaxIter)) .* Vector + FullArray;%Equation 2 and 6
                ...
                if ismember(i, idx1)
                    Candidate = BestPop +  sign(rand - 0.5) * (normrnd(1, 0.03)) .* (Pops(i, :) - BestPop);%Equation 7
                else
                    S = randperm(N, 2);
                    while any(S == i), S = randperm(N, 2); end
                    if fitness(S(1)) < fitness(S(2))%Equation 8
