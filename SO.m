function [BestScore, BestPop, conv_curve] = SO(N, MaxIter, Lb, Ub,dim, fojb)

% --- Initialization ---
Pops = Lb + (Ub - Lb) .* rand(N, dim);%Equation 1
...
for It = 1:MaxIter
    for i = 1:N
        if rand < rand * (1 - It / MaxIter)
            Candidate = (Ub + Lb + 2 .* BestPop - 2 .* Pops(i,:)) / 2 + ...
                        rand * abs(randn) * (BestPop - Pops(i, :)); %Equation 2
        else
            if rand > rand
                S = randperm(N, 2);
                ...
                Candidate = Pops(i, :)+(1 - exp(-5 * (It+20) / MaxIter)) .* Vector + FullArray;%Equation 6
                ...
                if ismember(i, idx1)
                    Candidate = BestPop +  sign(rand - 0.5) * (normrnd(1, 0.03)) .* (Pops(i, :) - BestPop);%Equation 8
                else
                    S = randperm(N, 2);
                    while any(S == i), S = randperm(N, 2); end
                    if fitness(S(1)) < fitness(S(2))%Equation 9
        ...
        %Boundary handling and fitness calculation
        Candidate = Boundary_processing(Candidate, Ub, Lb);
        CandidateFitness = fojb(Candidate);  

        %Ruthless pursuit of the best option
